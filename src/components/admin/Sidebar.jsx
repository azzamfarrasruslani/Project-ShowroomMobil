...
export default function Sidebar() {
  ...
  {!isCollapsed ? (
    <Link to="/">
      <img src="/image/Mobilin_Logo_1.png" alt="Logo" className="w-28" />
    </Link>
  ) : (
    <Link to="/">
      <img src="/image/Mobilin_Logo_1.png" alt="Logo" className="w-8" />
    </Link>
  )}
    ...
    {isCollapsed ? <FaChevronRight /> : <FaChevronLeft />}
      ...
      <div className="flex-1 space-y-6 overflow-y-auto">
        ...
        <Section title="Support" isCollapsed={isCollapsed}>
          ...
          />
          <NavItem
            to="/support/pengaturan"
            icon={<FaCogs className="text-pink-400" />}
            label="Pengaturan"
            isCollapsed={isCollapsed}
            hoverStyle={hoverStyle}
          />
      ...
      <div className={`mt-6 pt-4 border-t ${borderStyle} space-y-2`}>
        <button
          onClick={toggleTheme}
          className={`flex items-center gap-2 text-sm ${hoverStyle} px-2 py-1.5 rounded transition`}
        >
          {isDark ? <FaSun /> : <FaMoon />}
          {!isCollapsed && <span>{isDark ? "Tema Terang" : "Tema Gelap"}</span>}
        </button>
        {!isCollapsed && (
          <p className="text-xs text-gray-400">© 2025 Mobilin</p>
        )}
      </div>
    </aside>
  );
...
function Section({ title, isCollapsed, children }) {
  return (
    <div>
      {!isCollapsed && (
        <p className="text-xs text-gray-400 uppercase mb-2 pl-1">
          {title}
        </p>
      )}
...
function NavItem({ to, icon, label, isCollapsed, hoverStyle }) {
  return (
    <Link
      to={to}
      className={`flex items-center gap-3 px-2 py-2 text-sm rounded ${hoverStyle} transition`}
    >
      {icon}
