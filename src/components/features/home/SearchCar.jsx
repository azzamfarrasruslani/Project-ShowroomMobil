    ...
    <div className="mt-30 mb-10 min-h-100 bg-gray-100 p-10 text-gray-100">
      <div className="mb-10 rounded-lg border-gray-700 bg-gray-900 p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Left Section */}
          <div>
            <label className="block text-gray-300 mb-2">Pencarian Mobil</label>
            <input
              type="text"
            ...
            <div className="mt-4 grid grid-cols-4 gap-4">
              {["Pilihan Favorit", "Mobil Keluarga", "TDP Rendah", "KM Rendah"].map(
                (label) => (
                  <button
                    key={label}
            ...
            <p className="text-sm text-gray-400 mt-6">Pilih berdasarkan merek:</p>
            <div className="mt-2 grid grid-cols-6 gap-4">
              {["Honda", "Toyota", "Suzuki", "Nissan", "Mitsubishi", "Daihatsu"].map(
                (brand) => (
                  <div key={brand} className="text-center group">
                    <img
                      src={`/${brand.toLowerCase()}.png`}
                      alt={brand}
                      className="mx-auto h-10 w-10 group-hover:scale-110 transition-transform"
                    />
          ...
          <div className="rounded-md border border-gray-700 bg-gray-800 p-6 shadow-lg">
            <p className="mb-4 text-lg font-bold text-gray-100">Jual Mobil Anda</p>
            <label className="block text-gray-300 mb-2">Merek Mobil</label>
            ...
            <label className="block text-gray-300 mb-2">Model Mobil</label>
            <select className="mb-4 w-full rounded-md border border-gray-700 bg-gray-900 p-3 text-gray-100">
            ...
            <p className="mt-4 text-xs text-gray-400">
              Dengan melanjutkan, saya setuju dengan <span className="text-yellow-500">Kebijakan Privasi</span> & <span className="text-yellow-500">Ketentuan Penggunaan</span>.
            </p>
          </div>
        </div>
      </div>
